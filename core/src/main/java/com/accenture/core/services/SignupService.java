...
@Component(service = SignupService.class, configurationPolicy = ConfigurationPolicy.OPTIONAL)
...
public class SignupService {
    ...
    public @interface Cfg {
        @AttributeDefinition(name = "title", description = "The title text to display given successful signup")
        String title() default "Success!";

        @AttributeDefinition(name = "description", description = "The description text to display given successful signup")
        String description() default "You've been successfully signed up!";

        @AttributeDefinition(name = "path", description = "The full path (node name included) where you want sign ups persisted")
